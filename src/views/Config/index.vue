...
<template>
        ...
        <ul class="w-56 m-0 mr-3 menu bg-base-200 pt-14">
            <li v-for="item in menuList" :key="item.name">
                ...
                <a v-else @click="skip(item.path)"
                    :style="item.name == route.name ? 'background-color:rgba(12,12,12,0.2)' : ''">{{ item.meta!.title
                    }}</a>
            </li>
        </ul>
    ...
    <footer class="p-10 rounded footer footer-center bg-base-200 text-base-content">
        <!-- <nav class="grid grid-flow-col gap-4">
            <a class="cursor-pointer link link-hover text-inherit" target="_blank"
                href="https://1kw20.fun">行有不得，反求诸己</a>
        </nav>
        <nav>
            <a class="cursor-pointer link link-hover text-inherit" target="_blank"
                href="https://1kw20.fun">破山中贼易，破心中贼难</a>
        </nav>
        <nav>
            <div class="grid grid-flow-col gap-4">
                ...
                <a href="https://twitter.com/TaborSwift" target="_blank" class="cursor-pointer "><svg-icon
                        name="twitter"></svg-icon></a>
        ...
        <aside>
            ...
            <!-- <p class="p-0 m-0">Copyright © 2024 - All right reserved by Log1997</p> -->
        </aside>
    </footer>
</template>
